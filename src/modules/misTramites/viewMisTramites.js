...
export default class ViewMisTramites extends Component {
   ...
   async componentDidMount() {
      this.setState({
         listaTiposTramites: await API.getTipoTramite()
      });
   }
   ...
   mostrarAclaracion = async (value, index) => {
      let lista = await API.getRequisito(this.state.listaTiposTramites[index].id)
      if (lista != null) {
         this.setState({
            costo: this.state.listaTiposTramites[index].costo,
            dias: this.state.listaTiposTramites[index].dias,
            listaRequisitos: lista
         })
      }
      /* this.setState({
         aclaracion: this.state.listaTiposTramites[index].aclaracion,
         idTipoTramite: this.state.listaTiposTramites[index].id,
         listaRequisitos: []
      }) */
   }
   ...
   render() {
      return (
         <View style={{ flex: 1 }}>
            <Encabezado title="MIS TRAMITES" subtitle="MEJORANDO PARA" subtitle2="ATENDERLO MEJOR" navigation={this.props.navigation}/>
            <ScrollView style={{ flex: 1 }}>
               ...
               <View style={{ marginHorizontal: 20 }}>
                  <Dropdown
                     ...
                     fontSize={16}
                     textColor={"rgb(0, 0, 0)"}
                     data={this.state.listaTiposTramites}
