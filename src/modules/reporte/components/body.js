    ...
    if (this.state.image == null && this.state.images.length == 0) {
      ...
      if (inserto != null) {
        if (inserto.message == "ok") {
          ...
          Alert.alert(
            "Hubo una error al momento de enviar, inténtelo de nuevo por favor"
          );
          this.setState({ mostrarEnviar: true });
        }
      } else {
        Alert.alert(
          "No se pudo enviar su reporte, inténtelo de nuevo por favor"
        );
        this.setState({ mostrarEnviar: true });
      }
    }
    ...
    return (
      ...
      >
        ...
        {this.state.hayUbicacion ? (
          <TouchableOpacity style={styles.button3} onPress={this.mostrarMapa}>
            <Text style={styles.buttonText}>
              Ubicación asignada - Modificar
            </Text>
          </TouchableOpacity>
        ) : (
